...
const Cadastrar = () => {
    //constante que guarda o cadastro 
    const [nome, setNome] = useState("")
    const [curso, setCurso] = useState("")
    const [ira, setIra] = useState("")

    const navigate = useNavigate() //especificar a rota de destino como argumento

    function handleSubmit(event) {
        event.preventDefault()

        const novoAluno = {nome,curso,ira}
        axios.post("http://localhost:3001/aluno/register", novoAluno) //lugar da web em que vai estar o resultado final
        .then(
            (response)=>{
